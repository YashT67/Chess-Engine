...
static bool is_square_attacked(struct Board &board, int square) {

}

// Generate legal moves for rooks with color = 'active_color' and append it to the move list
static void gen_rook_moves(MoveList &move_list, struct Board &board) {
    bool color = board.active_color;
...
// Generate legal moves for bishops with color = 'active_color' and append it to the move list
static void gen_bishop_moves(MoveList &move_list, struct Board &board) {
    bool color = board.active_color;
...
// Generate legal moves for queens with color = 'active_color' and append it to the move list
static void gen_queen_moves(MoveList &move_list, struct Board &board) {
    bool color = board.active_color;
...
// Generate legal moves for knights with color = 'active_color' and append it to the move list
static void gen_knight_moves(MoveList &move_list, struct Board &board) {
    bool color = board.active_color;
...
// Generate legal moves for king with color = 'active_color' and append it to the move list
static void gen_king_moves(MoveList &move_list, struct Board &board) {
    bool color = board.active_color;
    ...
    if(is_square_attacked(board, from))
        return;
    if(board.kingside_castling[color]) {
        if(!((3ULL << (from + 1)) & board.common) && \
            !is_square_attacked(board, from + 1) && \
            !is_square_attacked(board, from + 2)) {
            move_list.add(encode_move(from, from+2, KING_CASTLE));
        }
    }
    if(board.queenside_castling[color]) {
        if(!((7ULL << (from - 3)) & board.common) && \
            !is_square_attacked(board, from - 1) && \
            !is_square_attacked(board, from - 2)) {
            move_list.add(encode_move(from, from-2, QUEEN_CASTLE));
        }
    }
}

// Generate legal moves for pawns with color = 'active_color' and append it to the move list
static void gen_pawn_moves(MoveList &move_list, struct Board &board) {
    bool color = board.active_color;
    ...
    if(color == WHITE) {
        ...
        Bitboard targets = board.by_color[BLACK] | ((board.en_passant == NO_SQ)? 0 : (1ULL << board.en_passant));
        ...
        while(left_capture) {
            ...
            int flag = (to == board.en_passant)? EN_PASSANT : CAPTURE;
            if(to >= 56) {
                flag |= 8;
        ...
        Bitboard targets = board.by_color[WHITE] | ((board.en_passant == NO_SQ)? 0 : (1ULL << board.en_passant));
...
MoveList gen_pseudo_legal_moves(struct Board &board) {
    ...
    // Rooks
    gen_rook_moves(move_list, board);

    // Bishops
    gen_bishop_moves(move_list, board);

    // Queens
    gen_queen_moves(move_list, board);

    // Knights
    gen_knight_moves(move_list, board);

    // King
    gen_king_moves(move_list, board);

    // Pawns
    gen_pawn_moves(move_list, board);
